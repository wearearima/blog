...
En un post anterior (//TODO){:target="_blank"} vimos cómo ha evolucionado la arquitectura de las aplicaciones. Con el paso de los años hemos pasado de desarrollar aplicaciones basadas en una arquitectura monolítica a aplicaciones basadas en (micro)servicios. Donde antes teníamos tests centralizados en una única aplicación, ahora pasamos a tenerlos divididos en varias. Hemos pasado por tanto a tener divididos los test para probar cada uno de los servicios de forma idenpendiente.
Derivada de esta división surge una nueva necesidad: asegurarnos que también la interacción entre los diferentes servicios es correcta. Esta necesidad podemos abordarla mediante test end-to-end, pero si bien estos tests ya resultan complejos de implementar/ejecutar en aplicaciones monolíticas en aquellas que no los son la complejidad es mayor.
En este punto, se nos plantea una duda: ¿no sería suficiente con verificar que los acuerdos/especificaciones para la comunicación entre consumer-producer se cumplen? Intentando resolver este “problema” descubrimos un nuevo concepto: Contract Testing.
...
Este concepto teórico en la práctica se traduce en lo siguiente:
...
Volvamos al ejemplo simplificado que hemos utilizado en el post anterior. 
